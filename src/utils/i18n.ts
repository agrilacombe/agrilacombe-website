 ...
 * Falls back to French if the key is not found in the target language,
 * then falls back to the key itself if not found in French.
 ...
 * French ('fr') is considered the default and resides at the root of the `site.base`.
 * Other languages are prefixed (e.g., /en/, /es/).
...
export function getLangBase(lang: Language, siteBase: string): string {
  ...
  // Due to redirects: { "/": "/fr/" }, all content is under a lang prefix.
...
export function getLocalizedPathForSwitcher(currentPath: string, targetLang: Language, currentLang: Language, siteBase: string): string {
  ...
  if (currentPath.startsWith(currentActualLangBase)) {
    ...
    // This case might occur if currentPath is the site root before Astro's redirect to /fr/
    // or if siteBase itself is complex and not handled by getLangBase as expected for `currentPath`.
    // Given the redirect "/": "/fr/", currentPath for French homepage should be /fr/ (or siteBase + /fr/).
    // If it's truly the root path '/' and currentLang is 'fr', the slug is effectively empty.
    const normalizedSiteRoot = siteBase.endsWith('/') ? siteBase : `${siteBase}/`;
    if (currentLang === 'fr' && currentPath === normalizedSiteRoot) {
      pageSlug = ''; // French root page slug is empty
    } else {
